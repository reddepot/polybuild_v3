# POLYBUILD v3 — Round 6 audit request (post-patch verification)

> **À toi, modèle audité** : tu fais partie d'une équipe de 6 LLM (Claude Opus 4.7,
> GPT-5.5, Gemini 3.1 Pro, Kimi K2.6, DeepSeek V4-Pro, Grok 4.20). Au round 5 vous
> avez collectivement renvoyé un **NO-GO unanime** (5 verdicts entre 5 et 6.5/10)
> sur le code POLYBUILD v3. J'ai intégré les corrections. **Round 6 = vérification
> ciblée, pas re-audit complet.**

---

## Ce qui s'est passé au round 5

Vous avez identifié **26 problèmes distincts**, classés par convergence :

### Convergences fortes (≥3 modèles d'accord — patches actés sans débat)
- **[A]** Phase 8 sans entrypoint CLI (3 modèles P0) → `python -m polybuild.phases.phase_8_prod_smoke` plantait sur `unrecognized arguments`.
- **[B]** Attestation type-cast unsafe (3 modèles P0/P1) → Pydantic ValidationError potentiel sur valeur YAML invalide.
- **[D]** MCP gate fragile (4 modèles) : stderr non drainé → deadlock possible ; lecture première ligne stdout fragile aux logs ; pas de matching JSON-RPC par `id`.

### Convergences moyennes (2 modèles)
- **[C]** eds-pseudo recréé à chaque appel (~350MB RAM spike) → singleton manquant.
- **[E]** `os.killpg` non portable Windows + race sans `await proc.wait()` → zombies + leaks.
- **[G]** `validate_rag` retournait `passed=True` quand rien n'était testé.
- **[H]** `validate_fts5` passait avec golden vide ou inexistant (skip masqué).
- **[J]** Qdrant gate cassait sur les collections à vecteurs nommés.
- **[K]** `deploy_staging.sh` lançait l'image Docker prod existante au lieu de builder depuis le worktree → faux staging.
- **[R]** Calcul p95 off-by-one (`int(0.95 * 20) = 19` = max, pas p95).
- **[S]** `notifications/initialized` envoyé sans `await drain()`.
- **[T]** `git reset --hard` sans guard `git status --porcelain` ni `git clean -fd`.

### Findings uniques mais légitimes (1 modèle, retenus)
- **[F]** SQLite `?mode=ro` seul casse sur WAL prod → ajout `&immutable=1`.
- **[I]** SQLite schema `changed` ne bloquait pas (seulement `removed`).
- **[L]** Bind Docker `0.0.0.0` exposait staging health-data sur le LAN.
- **[M]** SKILL.md utilisait `--spec`, cli.py acceptait `--brief` → CLI plantait.
- **[N]** Phase 9 cleanup hors finally global → leak worktree sur abort early.
- **[O]** `_invoke_role` Phase 5 violait BuilderProtocol → TypeError sur tous les 7 adapters.
- **[Q]** P3 utilisait `sem.locked()` qui ne capture que la saturation totale, pas la contention partielle promise.
- **[U]** Seuil `len(text) > 300` BLOCK trop strict (4 phrases bloquaient).
- **[V]** `.polybuild/runs/*.log` allowlistés gitleaks → fuites masquées.
- **[W]** `validate_qdrant_collection_sync` plantait si déjà dans event loop.
- **[X]** `duration_total_sec` figé avant Phase 7/8 → mesure fausse.
- **[Y]** Config path `parents[3]` cassait en wheel.
- **[Z]** Regex OpenAI matchait aussi OpenRouter (faux positif classification).

### Faux positif rejeté
- **[P]** "Trailing spaces dans Literals/strings" (Audit GPT-5.5 / round 5) — vérification : aucun espace parasite. **Hallucination.**

### Findings d'un seul audit qui remettent en cause des convergences round 4 — non actés (à débattre round 6)
- DeepSeek round 5 (Q5) : seuil dégradation Phase 8 = 1% + 2 échecs consécutifs au lieu de 0% strict.
- Gemini round 5 (Q5) : SQLite WAL bloquant universel à reconsidérer pour DBs immutables.
- Kimi round 5 (Q5) : `eds-pseudo` à éliminer entièrement faute de garantie de couverture.
- Grok round 5 (Q5) : DBs montées `:ro` à remplacer par snapshot copy avant Docker.
- ChatGPT round 5 : Allowlist gitleaks `.polybuild/runs/*.log` à retirer (✓ acté en [V]).

---

## Patches appliqués — récap technique

Tous les patches sont marqués `# Round 5 fix [<lettre>]` dans le code pour faciliter la revue.

| # | Fichier | Patch | Vérification |
|---|---|---|---|
| A | `phase_8_prod_smoke.py` | + `_amain()` argparse + `if __name__ == "__main__"` | `python -m … --help` retourne 0 ✓ |
| B | `phase_minus_one_privacy.py` | `_normalize_attestation()` + suppression de tous les `# type: ignore[arg-type]` | smoke test 6 cas OK |
| C | idem | `_get_eds_nlp()` singleton module-level avec gardes ImportError + load fail | 1 chargement max par run |
| D | `validate_mcp.py` | `_drain_stderr()` task + `_send_jsonrpc()` skip non-JSON + id matching | imports OK |
| E | idem | `sys.platform != "win32"` guard + `await proc.wait()` toujours | imports OK |
| F | `validate_sqlite.py` | `?mode=ro&immutable=1` | imports OK |
| G | `validate_rag.py` | refuse `passed=True` si aucun check configuré | imports OK |
| H | `validate_fts5.py` | rejette golden vide ou <3 queries ; skip explicite via `failures=["GOLDEN_SKIPPED_DEV_MODE"]` | imports OK |
| I | `validate_sqlite.py` | `changed` ajouté aux `errors` | imports OK |
| J | `validate_qdrant.py` | param `vector_name` + auto-detect + payload `{"name", "vector"}` | imports OK |
| K | `deploy_staging.sh` | `docker build -t … "${WORKTREE_DIR}"` + check Dockerfile present | bash -n OK |
| L | idem | `-p "127.0.0.1:${STAGING_PORT}:..."` | bash -n OK |
| M | `cli.py` + `SKILL.md` | `--spec`/`--brief` aliases + `EXTRA_ARGS=$*` dans skill | typer parse OK |
| N | `orchestrator.py` | `try/finally` global → `phase_9_cleanup` toujours appelé | refactor en `_run_polybuild_inner` |
| O | `builder_protocol.py` + `phase_5_triade.py` | méthode concrète `run_raw_prompt()` avec impl par défaut | imports OK, signature compat |
| Q | `concurrency/limiter.py` | `_inflight: dict[str, int]` + check `>0` au lieu de `sem.locked()` | smoke test OK : P3 drop sur partial contention |
| R | `phase_8_prod_smoke.py` | `math.ceil(0.95*n)-1` au lieu de `int(0.95*n)` | calcul correct |
| S | `validate_mcp.py` | `await proc.stdin.drain()` ajouté | imports OK |
| T | `phase_8_prod_smoke.py` | `git status --porcelain` guard + `git clean -fd` | imports OK |
| U | `phase_minus_one_privacy.py` | seuil 300→1500 chars + message clarifié | smoke test OK |
| V | `.gitleaks.toml` | retiré `.polybuild/runs/*.log` de l'allowlist | parse TOML OK |
| W | `validate_qdrant.py` | `try get_running_loop` puis raise explicite si déjà dans loop | imports OK |
| X | `orchestrator.py` | `run.duration_total_sec` recalculé après Phase 8 | imports OK |
| Y | `concurrency/limiter.py` | env var → cwd walk → fallback (4 candidats) | imports OK |
| Z | `.gitleaks.toml` | regex OpenAI avec `(?!(?:or-\|ant-))` lookahead négatif | parse TOML OK |

**Validation finale** : 36 fichiers Python parse OK, 6 YAML parse OK, 2 TOML parse OK, bash syntax OK, 12/12 modules importent OK, smoke tests fonctionnels passent.

---

## Ce que je te demande au round 6

**Audit ciblé sur les patches.** Pas de re-audit du squelette validé rounds 1-3. Pas de re-débat des décisions actées rounds 1-4.

### Q1 — Vérification des patches (le fix marche-t-il ?)
Pour chacun des 25 patches A-Z (sauf P rejeté), réponds : `OK` / `INSUFFISANT (raison)` / `RÉGRESSION (description)`.
Format compact :
```
[A] OK
[B] OK
[C] INSUFFISANT — le singleton n'est pas thread-safe; sur 2 runs polybuild parallèles via tmux ils se piétineront
[D] OK
...
```
Sois bref, sauf si tu détectes un problème.

### Q2 — Régressions introduites par les patches
Un patch peut casser autre chose. Liste explicitement :
- Nouveau couplage non documenté
- Effet de bord sur un autre module
- Changement de contrat silencieux
Format : `[<lettre>] → casse <module>:<ligne> car <raison>`

### Q3 — Findings résiduels que ROUND 5 a manqués
Maintenant que tu vois le code patché, vois-tu un bug que les 5 audits round 5 (toi inclus) ont manqué collectivement ?
Sois honnête sur tes propres oublis. P0/P1 prioritaires.

### Q4 — Convergences round 4 que TU remettais en cause au round 5 (Q5 du dernier prompt)
Au round 5, ton dossier Q5 demandait des contre-propositions sur les décisions round 4.
Maintiens-tu ta position vu le code patché, ou la nuances-tu ?
Si tu n'avais rien proposé en Q5 au round 5, tu peux passer cette question.

### Q5 — Verdict final GO/NO-GO sprint A
Au round 5 : **NO-GO unanime, score 5-6.5/10**. Réévalue.

```
Score round 6 : N/10
Δ vs round 5 : +X
Verdict : GO | NO-GO car <raison principale> | CONDITIONAL_GO si <X>
```

Si NO-GO encore : top 3 priorités résiduelles dans l'ordre.
Si GO : tu engages ta crédibilité. Si tu vois un risque mais que tu valides quand même, dis-le.

### Q6 — Ton propre travail au round 5
Méta-question (optionnelle, mais utile pour calibrer la suite) :
- Combien de tes findings round 5 étaient des hallucinations ? (Référence : Audit 2 GPT-5.5 a halluciné [P] trailing spaces.)
- Quelle convergence as-tu manqué qui s'est révélée critique chez les autres ?
...
```markdown
## Q1 — Vérification des patches
[A] OK
[B] OK
[C] OK | INSUFFISANT — ...
...

## Q2 — Régressions introduites
- [<lettre>] → ...

## Q3 — Findings résiduels
### [P0] <titre>
Fichier : ...
Patch : ...

## Q4 — Convergences round 4 reconsidérées
- ... (ou "rien à signaler")
...
1. **Brièveté.** Si le patch est OK, dis OK. Pas de paraphrase.
2. **Pas de re-audit complet.** Concentre-toi sur le delta.
3. **Si tu vois une régression**, ligne précise + raison concrète.
4. **Anti-sycophantie maintenue.** Si tu trouves le code maintenant correct, dis-le sans guirlandes.
5. **Voice notes interdites.** Markdown uniquement.

---

## Code patché à auditer

### Repository tree (round-5 patched files marked ★)
...
## Patched files (PRIMARY ROUND 6 AUDIT TARGET)

Look for `# Round 5 fix [<lettre>]` markers in the code.
...
### `src/polybuild/phases/phase_8_prod_smoke.py` (524 lines)
...
def rollback_to_tag(tag: str, repo_dir: Path | str = ".", force_clean: bool = True) -> bool:
    """Hard-reset to a tag and remove untracked artefacts.

    Round 5 fix (T):
      - Guard against destroying uncommitted local work (`git status --porcelain`).
        If non-empty AND `force_clean=False`, refuse to rollback.
      - After reset, run `git clean -fd` to remove untracked files (LLM-generated
        test files, caches, etc.) that `reset --hard` ignores.
    """
    ...
    # Remove untracked files (LLM-generated artefacts)
...
### `src/polybuild/phases/phase_6_validate.py` (284 lines)
...
### `src/polybuild/phases/phase_5_triade.py` (538 lines)
...
### `src/polybuild/orchestrator.py` (423 lines)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass  # checkpointing best-effort during cleanup
        except Exception as e:
            logger.warning("phase_9_cleanup_outer_finally_failed", error=str(e))
...
### `src/polybuild/cli.py` (210 lines)
...
### `src/polybuild/domain_gates/validate_mcp.py` (328 lines)
...
async def validate_mcp_server(
    ...
    golden_tool_call: dict | None = None,
) -> MCPGateResult:
...
### `src/polybuild/domain_gates/validate_sqlite.py` (151 lines)
...
### `src/polybuild/domain_gates/validate_qdrant.py` (168 lines)
...
async def validate_qdrant_collection(
    ...
    async with httpx.AsyncClient(timeout=timeout_s) as client:
        ...
        if not errors:
            ...
            search_payload: dict = {"limit": 3, "with_payload": False}
...
### `src/polybuild/domain_gates/validate_fts5.py` (158 lines)
...
### `src/polybuild/domain_gates/validate_rag.py` (148 lines)
...
### `src/polybuild/concurrency/limiter.py` (325 lines)
...
### `src/polybuild/adapters/builder_protocol.py` (119 lines)
...
class BuilderProtocol(ABC):
    ...
    async def run_raw_prompt(
        ...
        synthetic Spec+VoiceConfig; adapters MAY override for efficiency
        (e.g. direct chat completions API call without worktree creation).
        """
        ...
        synthetic_cfg = VoiceConfig(
            ...
            context={"phase5_workdir": str(workdir), "raw_prompt": True},
        )
...
### `scripts/deploy_staging.sh` (138 lines)
...
### `skills/polybuild/SKILL.md` (187 lines)
...
# Round 5 fix [M]: pass all extra args ("$@" minus first $1) so --profile etc.
# get forwarded to `polybuild run`. Previous version dropped them silently.
SPEC="$1"; shift
EXTRA_ARGS="$*"
...
if command -v tmux >/dev/null 2>&1; then
  tmux new-session -d -s "polybuild-${RUN_ID}" \
    "set -a; \
     ...
     set +a; \
     uv run polybuild run --spec '${SPEC}' --run-id '${RUN_ID}' ${EXTRA_ARGS} \
  ...
  screen -dmS "polybuild-${RUN_ID}" \
    bash -c "set -a; [ -f \"\$HOME/.polybuild/secrets.env\" ] && . \"\$HOME/.polybuild/secrets.env\"; set +a; \
             uv run polybuild run --spec '${SPEC}' --run-id '${RUN_ID}' ${EXTRA_ARGS} 2>&1 | tee '.polybuild/logs/${RUN_ID}.log'"
  echo "${RUN_ID}" > .polybuild/last_run
...
### `.gitleaks.toml` (78 lines)

```toml
# .gitleaks.toml — POLYBUILD secrets policy (Round 4 + Round 5 finalisé)
#
# Round 5 fixes:
...
#       transiter — les exclure du scan = invisibilité maximale. Les logs
#       restent gitignorés (ne devraient jamais être commités) mais s'ils le
#       sont par erreur, on veut un blocage gitleaks, pas un pass silencieux.
...
[allowlist]
description = "Allow only the local secrets file (chmod 600). Logs NOT exempted."
paths = [
...
## Critical dependencies (context only — do NOT re-audit)
...
**End of code.** Now answer Q1-Q6 in the format specified above.
Be brief, specific, and honest. If your verdict moves from NO-GO to GO,
say so plainly. If it stays NO-GO, name the residual blocker. Thanks.
