# POLYBUILD v3 — Round 9 hybrid audit (verification + pre-mortem cont'd)
...
### `src/polybuild/phases/phase_5_triade.py` (583 lines)
...
### `src/polybuild/phases/phase_2_generate.py` (140 lines)
...
async def phase_2_generate(
    ...
    async def _bounded_generate(
        cfg: VoiceConfig, builder: object
    ) -> BuilderResult:
        ...
        return await limiter.run(
            ...
            coro_factory=lambda b=builder, s=spec, c=cfg: b.generate(s, c),  # type: ignore[attr-defined]
...
### `src/polybuild/phases/phase_7_commit.py` (354 lines)
...
async def phase_7_commit(
    ...
    if winner_result is not None and winner_result.code_dir.exists():
        ...
        if (
            ...
            and winner_result.tests_dir.is_relative_to(  # type: ignore[attr-defined]
...
### `src/polybuild/phases/phase_minus_one_privacy.py` (491 lines)
...
### `src/polybuild/orchestrator.py` (488 lines)
