...
**Version courante :** **v3.2.6** (stable). 607 tests passed, ruff +
mypy --strict clean, bandit clean, pip-audit clean, detect-secrets
clean. Production-ready ; le prochain bottleneck est l'usage réel sur
un brief, pas l'audit statique.

## Méta : équipe de conception

POLYBUILD a été conçu par consultation multi-LLM en 4 rounds, puis
durci par 5 audits POLYLENS croisant 17+ voix indépendantes (Gemini,
GPT-5.5, Codex, Kimi, Qwen, GLM, Grok, DeepSeek, MiniMax, MiMo,
Claude). L'équipe de design réunissait 6 modèles avec diversité 5D
(provider, architecture, alignment, corpus, role bias) :
...
Sémaphores asyncio par provider. Limites médianes (à confronter à
l'usage réel) :
...
│   └── adr.md                      # phase 7
├── scripts/
│   └── deploy_staging.sh           # worktree + Docker RO + smoke + rollback
├── skills/polybuild/
│   └── SKILL.md                    # tmux/screen/nohup chain
└── src/polybuild/
...
## Risques résiduels (à valider empiriquement)
...
- Publier la lib `devcode` sur PyPI (aujourd'hui sibling project local).
- Mesurer K3/K6 sur runs réels (overhead `--solo` vs `--consensus`,
  taux de divergence DEVCODE vs naïf).
- Domain gates additionnels : `mutation_testing` pour
  `module_inedit_critique`, `property_tests_hypothesis` +
  `numerical_invariants` pour `helia_algo`.
- Consultation CIL/DPO sur Phase -1 Privacy Gate (recommandé — LLMs
  pas compétents juridiquement).
